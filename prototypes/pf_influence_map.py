...
class InfluenceMap(pf_map_base.MapBase):
	...
	def __expand_loop(self, area_id, loop):
		...
		while not update_loop.is_finished():
			...
			ipp_annhilated = (new_b == edge_ipp.b)
			...
			ipppp_annhilated = ipp_annhilated and (edge_ipppp.b == new_a)
			...
			if immmm_annhilated or ipppp_annhilated:
			...
			if ipp_annhilated:
				...
				# if we also have to delete the other edge
				if ipppp_annhilated and not immmm_annhilated:
					update_loop.delete(+1 -1)
			else:
...
class PseudoInPlaceUpdateArray:
	...
	def get(self, rel_pos):
		"""
			Get the element, it is addressed relativ to the current one,
